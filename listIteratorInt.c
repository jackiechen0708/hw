...
int add(IteratorInt iter, int v){
  ...
  if(iter->head == NULL) {
    ...
    // link the tmp node to the previous node which is currently the tail element.
    iter->prevNode->next = tmp;
    tmp->prev = iter->prevNode;
    iter->prevNode = tmp;		// moving cursor (Only need to move prevNode as its end of the list and nextNode will still point to NULL)
    iter-> size++;              //showLL(iter->head);
    return 1;
  }
...
void freeIt(IteratorInt iter){
  ...
  assert(iter!=NULL);		// check current iterator
  Node *curr, *prev;		// Initialize pointers
  curr = iter->head;		// make pointer point to head variable
